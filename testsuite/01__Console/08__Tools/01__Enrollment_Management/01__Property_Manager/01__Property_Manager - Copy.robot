...
*** Test Cases ***
Property Manger Creatiton
    [Tags]
    ${worksheet}=    Evaluate    'property_manager'
    ${property_manager_subject_attribute_col}=    Evaluate    1
    Open Excel    ${workbook}
    ${nrows}=    Get Row Count    ${worksheet}
    : FOR    ${row}    IN RANGE    1    ${nrows}
    \    ${property_manager_subject_attribute}=    Read Cell Data By Coordinates    ${worksheet}    ${property_manager_subject_attribute_col}    ${row}
    \    Go To    ${Home}/${Property Manager List}
    \    Maximize Browser Window
    \    Wait Until Page Contains Element    ${add property button}    #ADD PROPERTY BUTTON
    \    Add Attributes    ${property_manager_subject_attribute}
    \    Sleep    2
